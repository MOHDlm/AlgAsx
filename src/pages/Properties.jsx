...
export default function PropertiesPage() {
  ...
  // 🛰️ جلب الحملات من البلوكشين (بدون MetaMask)
  useEffect(() => {
    const fetchCampaigns = async () => {
      try {
        setIsLoading(true);
        console.log("⏳ Fetching campaigns (Read-Only)...");

        // ✅ استخدام getFactoryContract بدون معاملات
        ...
        if (!campaignsData || campaignsData.length === 0) {
          console.warn("⚠️ No campaigns found on blockchain");
          setProperties([]);
          return;
        }
  ...
  const filteredProperties = properties
    .filter(
      (property) => property.contract && property.contract.startsWith("0x")
    )
    .filter((property) => {
